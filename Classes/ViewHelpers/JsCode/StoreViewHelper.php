...
class Tx_MvcExtjs_ViewHelpers_JsCode_StoreViewHelper extends Tx_MvcExtjs_ViewHelpers_JsCode_AbstractJavaScriptCodeViewHelper {
	 ...
	 * @param $reader the reader for the store
	 * @param $writer the writer for the store
	 * @param $proxy the proxy for the store
	 * @param $data the data for the store
	 * @return void
	 */
	...
	private function renderReaderVariable($class) {
		...
		$jsConstructor->setObjectName('Ext.data.JsonReader');
			...
			// Make config
		$jsConstructor->set('totalProperty', 'total')
					  ->set('successProperty', 'success')
					  ->set('idProperty', 'uid')
					  ->set('root', 'data');

		$fieldArray = Tx_MvcExtjs_ExtJS_Utility::getFieldsArray($class);
		$jsConstructor->setRaw('fields', $fieldArray);
	...
	private function renderProxyVariable($controller = NULL, array $actions = array()) {
		...
		$jsConstructor->setRaw('api',$apiObject);
		$jsOut = $jsConstructor->build();
		return $jsOut;
	}
	...
	private function renderWriterVariable() {
		...
		$jsConstructor->setRaw('encode', 'true')
					  ->setRaw('writeAllFields', 'false');
		$jsOut = $jsConstructor->build();
		return $jsOut;
	}
